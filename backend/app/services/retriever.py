...
class Retriever:
    ...
    def __init__(self):
        logger.debug(f"🔄 Loading embedding model: paraphrase-multilingual-MiniLM-L12-v2")
        self.model = SentenceTransformer(
            "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
        )
        logger.debug(f"✅ Embedding model loaded")
        ...
        for idx in indices[0]:
            if idx < len(self.chunks):
                ...
                if country and country != "latam" and chunk.get("country") not in [country, "latam"]:
                    logger.debug(f"⏭️ Skipping chunk {chunk.get('chunk_id')} - country mismatch")
                    continue
